...
int input_check(int argc, char **argv, char **commit_msg);
int build_tree_pass_one(char *index_path, node *root);
int	build_tree_second_pass_recursive_ascent(node *root);
int accept_commit(node *root, const char *commit_msg, char out[SHA1_HEX_BUFFER_SIZE]);
...
static int read_tree_hash_from_commit(const char *commit_hash, char **tree_hash) {
    ...
    space = first_line + 5;
    *tree_hash = duplicate_string(space);
    free(first_line);
    if (*tree_hash == NULL) {
        return (-1);
    }
...
int accept_commit(node *root, const char *commit_msg, char out[SHA1_HEX_BUFFER_SIZE]){
    ...
    if (build_commit_payload(root->hash, branch_name, current_commit_hash, commit_msg, &payload) == -1) {
        goto cleanup;
    }
